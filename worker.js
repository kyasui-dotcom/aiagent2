...
<section class="main"><section class="box"><div class="section-title">AGENT REGISTRY</div><div id="agents"></div></section><section class="box"><div class="section-title">RECENT JOBS</div><div id="jobs"></div></section></section><section class="main"><section class="box"><div class="section-title">JOB DETAIL</div><pre id="jobDetail" class="muted">Select a recent job by clicking RUN MATCH or SEED DEMO.</pre></section><section class="box"><div class="section-title">MVP STATUS</div><div class="muted">Worker-backed API active. Jobs, events, billing, and agent matching now execute through real endpoints. Next step: bind D1 and persist this state.</div></section></section>
</main>
<script>
const el={stream:document.getElementById('stream'),active:document.getElementById('activeJobs'),online:document.getElementById('onlineAgents'),cost:document.getElementById('todayCost'),rev:document.getElementById('platformRevenue'),agents:document.getElementById('agents'),jobs:document.getElementById('jobs'),jobDetail:document.getElementById('jobDetail')};
...
const state = {
  agents: [
    { id: 'agent_research_01', name: 'RESEARCH_01', description: 'Market research and comparison.', taskTypes: ['research', 'summary'], premiumRate: 0.2, basicRate: 0.1, successRate: 0.94, avgLatencySec: 18, online: true, token: 'secret_research_01', earnings: 0, owner: 'Kuni', manifestUrl: 'https://github.com/ichigoichie-ai/agent-broker/blob/main/agents/research.json' },
    { id: 'agent_writer_01', name: 'WRITER_01', description: 'LP and copy drafts.', taskTypes: ['writing', 'summary'], premiumRate: 0.1, basicRate: 0.1, successRate: 0.91, avgLatencySec: 25, online: true, token: 'secret_writer_01', earnings: 0, owner: 'Kuni', manifestUrl: 'https://github.com/ichigoichie-ai/agent-broker/blob/main/agents/writer.json' },
    { id: 'agent_code_01', name: 'CODE_01', description: 'Bugfix and implementation tasks.', taskTypes: ['code', 'debug'], premiumRate: 0.3, basicRate: 0.1, successRate: 0.89, avgLatencySec: 40, online: true, token: 'secret_code_01', earnings: 0, owner: 'Kuni', manifestUrl: 'https://github.com/ichigoichie-ai/agent-broker/blob/main/agents/code.json' },
    { id: 'agent_seo_01', name: 'SEO_01', description: 'SEO briefs and metadata.', taskTypes: ['seo', 'summary', 'writing'], premiumRate: 0.15, basicRate: 0.1, successRate: 0.9, avgLatencySec: 15, online: true, token: 'secret_seo_01', earnings: 0, owner: 'Kuni', manifestUrl: 'https://github.com/ichigoichie-ai/agent-broker/blob/main/agents/seo.json' },
    { id: 'agent_listing_01', name: 'LISTING_01', description: 'Product listing optimization.', taskTypes: ['listing', 'writing', 'summary'], premiumRate: 0.12, basicRate: 0.1, successRate: 0.92, avgLatencySec: 16, online: true, token: 'secret_listing_01', earnings: 0, owner: 'Kuni', manifestUrl: 'https://github.com/ichigoichie-ai/agent-broker/blob/main/agents/listing.json' },
    { id: 'agent_ops_01', name: 'OPS_01', description: 'Task routing and workflow ops.', taskTypes: ['ops', 'summary', 'research'], premiumRate: 0.18, basicRate: 0.1, successRate: 0.95, avgLatencySec: 12, online: true, token: 'secret_ops_01', earnings: 0, owner: 'Kuni', manifestUrl: 'https://github.com/ichigoichie-ai/agent-broker/blob/main/agents/ops.json' }
  ],
  jobs: [],
  events: []
};
...
export default {
  async fetch(request, env) {
    ...
    if (request.method === 'GET' && url.pathname.match(/^\/api\/jobs\/[^/]+$/)) { const id = url.pathname.split('/')[3]; const job = state.jobs.find(j => j.id === id); return job ? json(job) : json({ error: 'Job not found' }, 404); }
