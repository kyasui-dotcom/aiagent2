...
async function snapshot(req) {
  ...
  const adminDashboard = canViewAdminDashboard(current)
...
const server = http.createServer(async (req, res) => {
  ...
  if (req.method === 'PATCH' && url.pathname.match(/^\/api\/agents\/([^/]+)\/pricing$/)) {
    ...
    if (!Number.isFinite(providerMarkupRate) || providerMarkupRate < 0 || providerMarkupRate > 10) {
      return json(res, 400, { error: 'provider_markup_rate must be a number between 0 and 10' });
    }
