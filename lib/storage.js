...
export function createD1LikeStorage(db) {
  if (!db) {
    const mem = { agents: structuredClone(DEFAULT_AGENT_SEEDS), jobs: [], events: [makeEvent('LIVE', 'broker storage initialized')] };
    return {
      kind: 'd1-ready',
      supportsPersistence: false,
      schemaSql: D1_SCHEMA_SQL,
      async getState() { return structuredClone(mem); },
      async replaceState(nextState) { mem.agents = nextState.agents; mem.jobs = nextState.jobs; mem.events = nextState.events; return structuredClone(mem); },
      async mutate(mutator) { const draft = structuredClone(mem); const result = await mutator(draft); mem.agents = draft.agents; mem.jobs = draft.jobs; mem.events = draft.events; return result; },
  ...
  async function upsertJob(job) {
    ...
    await db.prepare(`INSERT OR REPLACE INTO jobs (id,parent_agent_id,task_type,prompt,input_json,budget_cap,deadline_sec,priority,status,assigned_agent_id,score,usage_json,billing_estimate_json,actual_billing_json,output_json,failure_reason,logs_json,created_at,started_at,completed_at,failed_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`)
      .bind(v.id, v.parent_agent_id, v.task_type, v.prompt, v.input_json, v.budget_cap, v.deadline_sec, v.priority, v.status, v.assigned_agent_id, v.score, v.usage_json, v.billing_estimate_json, v.actual_billing_json, v.output_json, v.failure_reason, v.logs_json, v.created_at, v.started_at, v.completed_at, v.failed_at)
      .run();
  }
