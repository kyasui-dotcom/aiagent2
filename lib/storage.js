...
import { DEFAULT_AGENT_SEEDS, DEPRECATED_AGENT_SEED_IDS, makeEvent, nowIso } from './shared.js';
...
export function createD1LikeStorage(db, options = {}) {
  ...
  function cloneCachedState() {
    return cachedState ? structuredClone(cachedState) : null;
  }

  function cacheState(state = null) {
    cachedState = state ? structuredClone(state) : null;
    cachedStateAt = cachedState ? Date.now() : 0;
  }
  ...
  function hasFreshStateCache() {
    return Boolean(cachedState && stateCacheTtlMs > 0 && (Date.now() - cachedStateAt) < stateCacheTtlMs);
  }
  ...
  if (!db) {
    ...
    return {
      ...
      async replaceState(nextState) {
        ...
        inMemoryState.jobs = nextState.jobs;
      ...
      async appendChatTranscript(transcript) {
        inMemoryState.chatTranscripts.unshift(transcript);
        if (inMemoryState.chatTranscripts.length > CHAT_TRANSCRIPT_RETENTION_LIMIT) inMemoryState.chatTranscripts = inMemoryState.chatTranscripts.slice(0, CHAT_TRANSCRIPT_RETENTION_LIMIT);
        cacheState(inMemoryState);
        return transcript;
      },
  ...
  return {
    ...
    async getState() {
      ...
      try {
        return structuredClone(await stateLoadPromise);
      } finally {
        stateLoadPromise = null;
      }
    },
    async replaceState(nextState) {
      ...
      for (const account of mergedAccounts) await upsertAccount(account);
