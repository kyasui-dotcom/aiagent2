...
function normalizeCheckoutReturnTab(value = 'settings') {
  const safe = normalizeString(value).toLowerCase();
  return ['work', 'settings'].includes(safe) ? safe : 'settings';
}

export async function createDepositCheckoutSession(config, { account, customerId, amount, currency = 'USD', baseUrl = '', ledgerAmount = null, returnTab = 'settings' }) {
  ...
  if (amountMinor <= 0) throw stripeApiError('Prepaid balance amount must be greater than zero', 400);
  const root = normalizeString(baseUrl || config.baseUrl);
  const safeReturnTab = normalizeCheckoutReturnTab(returnTab);
  const metadata = buildMetadata(account, {
    ...
    aiagent2_return_tab: safeReturnTab,
  ...
  return stripeRequest(config, {
    ...
    form: {
      mode: 'payment',
      customer: customerId,
      success_url: `${root}/?tab=${safeReturnTab}&stripe=prepaid_success`,
      cancel_url: `${root}/?tab=${safeReturnTab}&stripe=prepaid_cancel`,
      line_items: [
        {
          quantity: 1,
          price_data: {
            ...
            product_data: { name: `CAIt prepaid balance (${safeCurrency})` }
          }
        }
      ],
      payment_intent_data: {
...
export async function createOffSessionTopupPaymentIntent(config, { account, customerId, paymentMethodId, amount, currency = 'USD', ledgerAmount = null }) {
  const safeCurrency = normalizeCurrency(currency, config.defaultCurrency || 'USD');
  const amountMinor = amountToMinorUnits(amount, safeCurrency);
  if (amountMinor <= 0) throw stripeApiError('Automatic billing amount must be greater than zero', 400);
  const metadata = buildMetadata(account, {
    aiagent2_kind: 'auto_topup_charge',
    aiagent2_amount_minor: String(amountMinor),
    aiagent2_currency: safeCurrency,
    ...(ledgerAmount != null ? { aiagent2_ledger_amount: String(ledgerAmount) } : {})
  });
