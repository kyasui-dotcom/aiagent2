...
function appendWorkflowEvidenceMarkdown(kind = '', body = {}, markdown = '', isJapanese = false) {
  ...
  if (/Specialist evidence used|使用した専門エージェントの根拠/i.test(text)) return text;
  const lines = [
    isJapanese ? '## 使用した専門エージェントの根拠' : '## Specialist evidence used',
    '| Task | Status | Summary | Key bullets | Files |',
    '| --- | --- | --- | --- | --- |'
  ];
  for (const run of runs.slice(0, 12)) {
    ...
    const files = Array.isArray(run.files)
      ? markdownTableCell(run.files.filter(Boolean).slice(0, 3).join(', '), 180)
      : '';
...
function leaderActionProtocolInstruction(body = {}, kind = '') {
  ...
  const priorInstruction = priorHint
    ? ` Completed specialist evidence available in input._broker.workflow.leaderHandoff.priorRuns: ${priorHint}. Cite these task names and use them before making a decision.`
    : '';
...
function webSearchSourceInstruction(kind = '', body = {}) {
  ...
  return 'Web search is enabled for this run. For current, competitor, SERP, pricing, channel, platform, or market claims, use the web_search tool first. Include observation date, source titles, and URLs in file_markdown. If the API returns no source URLs, explicitly label those claims as not source-verified instead of saying research was completed.';
}
...
async function callOpenAi(kind, body, source = {}) {
  ...
  const requestOptions = structuredRequestOptions(kind, body);
