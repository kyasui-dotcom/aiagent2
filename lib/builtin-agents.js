...
## 先に結論
${context.product} の初手は、広告や広いSNS告知ではなく **エンジニア向けの比較/SEO LP** です。登録CVが目的で${context.budget}なら、まず「なぜ登録するのか」を検索意図に合わせて説明し、流入後のCTAを短くします。
...
function shouldAppendPolicySectionsToFile(kind = '') {
...
export function sampleAgentPayload(kind, body = {}) {
  ...
  const finalMarkdown = shouldAppendPolicySectionsToFile(kind) ? withDeliveryQualityMarkdown(
    kind,
