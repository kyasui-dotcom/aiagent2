...
function cmoWorkflowEnglishSpecialistMarkdown(kind = '', context = {}) {
  const normalizedKind = String(kind || '').trim().toLowerCase();
  const date = nowIso().slice(0, 10);
  if (normalizedKind === 'research') {
    return `# CMO acquisition research delivery

## Answer first
For ${context.product}, the first executable acquisition lane is an engineer-focused comparison/SEO landing path, then one small social/community validation loop after the destination is ready. The goal is ${context.conversion}, the ICP is ${context.icp}, and the budget constraint is ${context.budget}.
...
## Decision
| Lane | Decision | Reason | Work product |
| --- | --- | --- | --- |
| Comparison/SEO LP | Use first | Closest to pre-signup comparison intent and durable without ads | /compare-ai-agents-for-engineers outline, FAQ, CTA |
| X | Use as validation after LP | Fast wording feedback and referral signal | One founder post with UTM |
| Reddit | Conditional | Works only as discussion-first, not link drop | One feedback post draft |
| Indie Hackers | Conditional | Fits build-in-public learning | One progress post draft |
| Paid ads | Hold | Bad fit before signup reason and tracking are fixed | Do not run |

## Execution packet
| Field | Value |
| --- | --- |
| Owner | CMO leader -> SEO/landing/growth |
| Objective | Explain why engineers should register before adding more traffic |
| Artifact | Comparison LP outline, CTA copy, FAQ, one post-ready social packet |
| Connector path | Publish/post only after connector access and explicit approval; otherwise return manual handoff |
| Metric | referral sessions, Work Chat starts, signup_started, signup_completed |
| Stop rule | If clicks do not create signup starts within 7 days, fix LP/CTA/signup reason before adding channels |

## Handoff to leader
Use teardown, data_analysis, media_planner, and action specialists to turn this lane into publish-ready artifacts. Do not stop at another research approval request.`;
  }
  if (normalizedKind === 'teardown') {
    return `# CMO competitor teardown delivery

## Answer first
${context.product} should not be positioned as another AI-agent directory. The sharper wedge is: an engineer-focused marketplace where discovery, order preparation, execution tracking, and delivery review stay in one workflow.

## Competitive frame
| Alternative | Type | Why users choose it | CAIt wedge | Proof needed |
| --- | --- | --- | --- | --- |
| AI tool / agent directories | Direct comparison | Many tools and categories | Workflow after discovery: order brief, run, delivery review | Example order and delivery screenshot |
| Developer automation platforms | Adjacent substitute | More technical control | Faster path for users who do not want to build orchestration | 3-step workflow proof |
| Search/X/manual discovery | Status quo | Free and familiar | Saved history, comparison, order trail, reusable agents | Signup benefits before CTA |

## Positioning decision
| Field | Value |
| --- | --- |
| ICP | Engineers evaluating, using, or publishing AI agents |
| Main promise | Compare agents, prepare an order, and keep delivery review in one place |
| Signup reason | Saved chats, order history, publishing flow, API/key management |
| First asset | Engineer comparison LP |

## Execution packet
| Field | Value |
| --- | --- |
| Owner | CMO leader -> landing/seo_gap |
| Artifact | Competitive comparison section for /compare-ai-agents-for-engineers |
| Copy angle | Not just a list: compare, order, and review delivery in one workflow |
| CTA | Sign up to use agents |
| Metric | Comparison LP CTR and signup_completed |
| Stop rule | If competitor objection repeats, add proof/examples before adding new channels |

## Handoff to leader
Release landing/seo_gap and growth specialists with this comparison frame. Current competitor pages should be verified with web search when available; unverified claims must stay labeled as assumptions.`;
  }
  if (normalizedKind === 'data_analysis') {
    return `# CMO data analysis delivery

## Answer first
Before judging channels, fix the signup funnel measurement. Google referrals without signups can mean weak intent, weak signup reason, login friction, or missing events; these must be separated.

## Funnel contract
| Step | Event | Denominator | Use |
| --- | --- | --- | --- |
| Visit | page_view / session_start | sessions | Landing traffic |
| Interest | work_chat_started | page_view | Work Chat pull |
| Order prep | draft_order_created | work_chat_started | Intent quality |
| Signup start | signup_started | page_view or draft_order_created | Auth entry |
| Signup complete | signup_completed | signup_started | Registration conversion |
| Paid/order | order_sent / paid_order_completed | draft_order_created | Monetization path |

## Execution packet
| Field | Value |
| --- | --- |
| Owner | CMO leader -> data_analysis |
| Artifact | 7-day and 14-day source/landing/signup funnel table |
| Required dimensions | source/medium, referrer, landing_page, auth_provider, device |
| Internal exclusions | operator email, admin paths, preview paths, known bot user agents |
| Metric | signup_started, signup_completed, Work Chat start rate |
| Stop rule | Do not add channels while signup events or admin counts are inconsistent |

## Handoff to leader
Give landing/growth the exact events above. If analytics is not connected, label it as unconnected instead of inventing numbers.`;
  }
  if (normalizedKind === 'media_planner') {
    return `# CMO media planner delivery

## Answer first
Use the comparison LP as the destination, then run one small validation post per channel. Do not send traffic to a broad homepage that does not explain the signup reason.

## Priority queue
| Priority | Channel | Action | Destination |
| --- | --- | --- | --- |
| 1 | SEO/comparison | Build the comparison LP and FAQ | /compare-ai-agents-for-engineers |
| 2 | X | Founder post with feedback question | LP with utm_source=x |
| 3 | Reddit | Discussion-first question | Comment link only if requested |
| 4 | Indie Hackers | Build-in-public learning post | LP with utm_source=indiehackers |

## Execution packet
| Field | Value |
| --- | --- |
| Owner | CMO leader -> growth/x_post/reddit/indie_hackers |
| Artifact | Channel queue, UTM map, exact post drafts |
| UTM | utm_medium=community, utm_campaign=cait_comparison_lp |
| Approval gate | exact copy, URL, UTM, posting account |
| Metric | qualified clicks, signup_started, signup_completed |
| Stop rule | If clicks arrive without signup starts, fix LP/CTA before posting more |

## Handoff to leader
Release action specialists only for the first destination and first post packet.`;
  }
  if (normalizedKind === 'x_post') {
    return `# CMO X post execution delivery

## Answer first
This is an approval-ready X post packet. It should not be published until the X connector/account and exact URL are approved.

## Post draft
I am building CAIt, an AI-agent marketplace for engineers.

The part I am testing now is not “more agents in a list.” It is whether discovery, order prep, execution tracking, and delivery review should live in one workflow.

Before signing up, what proof would make you try an AI agent marketplace: examples, pricing, ordering flow, or publishing your own agent?

## Execution packet
| Field | Value |
| --- | --- |
| connector | X |
| action | post after approval |
| URL | /compare-ai-agents-for-engineers?utm_source=x&utm_medium=community&utm_campaign=cait_comparison_lp |
| approval owner | Operator or CMO leader |
| metric | clicks, replies, signup_started, signup_completed |
| stop rule | If replies ask “why sign up?”, improve signup reason before posting again |

## Handoff to leader
Request X connector approval or present this as a manual posting packet.`;
  }
  if (normalizedKind === 'reddit') {
    return `# CMO Reddit execution delivery

## Answer first
This should be a discussion-first Reddit packet, not a direct promotional link drop.

## Post draft
Title: What proof would make an AI agent marketplace useful before signup?

Body:
I am building an AI-agent marketplace for engineers. The challenge is not the list of agents; it is explaining what registration unlocks before someone tries the workflow.

Would examples, transparent pricing, ordering flow, or publishing your own agent matter most before signup?
...
| connector | Reddit |
| action | draft / submit after approval |
| URL policy | Share the comparison LP only if the discussion asks for it |
| approval owner | Operator or CMO leader |
| metric | comment quality, qualified clicks, signup_completed |
| stop rule | Stop linking if the thread reads as promotion |

## Handoff to leader
Select subreddit and approval owner before posting.`;
  }
  if (normalizedKind === 'indie_hackers') {
    return `# CMO Indie Hackers execution delivery

## Answer first
Use a learning-oriented build-in-public post about signup reasons, not a generic launch announcement.

## Post draft
Title: I learned that traffic is not enough without a signup reason

Post:
I am building CAIt, an AI-agent marketplace for engineers. The current lesson is that a marketplace homepage must explain what registration unlocks before asking for signup: saved work, order history, delivery review, and publishing your own agent.

This week I am testing whether a comparison page converts better than a broad homepage.
...
| CTA | feedback request, not hard sell |
| metric | comments, profile/site clicks, signup_completed |
| stop rule | If no useful comments, shift the next post to “how ordering works” |

## Handoff to leader
Approve exact copy and destination URL before posting.`;
  }
  return `# CMO growth execution delivery

## Answer first
The first execution sprint is: build the comparison LP, prepare one social/community validation packet, then read signup events. Do not continue with another broad strategy memo.

## 7-day execution sprint
| Day | Action | Artifact |
| --- | --- | --- |
| 1 | Draft comparison LP headline, CTA, FAQ | LP packet |
| 2 | Add signup-reason copy near CTA | Replacement copy |
| 3 | Prepare one X/community post | Approval-ready post |
| 4 | Publish/post only if approved | URL or manual proof |
| 5 | Read signup_started and signup_completed | Funnel table |
| 6 | Fold response language back into LP | Revised copy |
| 7 | Decide one continuation lane | Next execution queue |

## Execution packet
| Field | Value |
| --- | --- |
| Owner | CMO leader -> growth |
| Objective | Turn the first acquisition lane into executable artifacts |
| Artifact | LP packet, social post packet, funnel readout |
| Connector path | Connector or manual handoff after approval |
| Metric | referral sessions, Work Chat starts, signup_started, signup_completed |
| Stop rule | If no signup movement, fix LP/CTA/signup reason before adding channels |

## Handoff to leader
Integrate this into the final CMO delivery as executed, approval-ready, or blocked by missing connector.`;
}
...
function cmoWorkflowSpecialistDelivery(kind = '', fallbackPrompt = '', body = {}, isJapanese = false) {
  ...
  let markdown = '';
  if (!isJapanese) markdown = cmoWorkflowEnglishSpecialistMarkdown(normalizedKind, context);
  else if (normalizedKind === 'research') markdown = cmoWorkflowResearchMarkdown(context);
...
function cmoWorkflowDeliveryQualityFailure(kind = '', body = {}, markdown = '', _isJapanese = false, fallbackPrompt = '') {
  if (!cmoWorkflowQualityGateApplies(kind, body, fallbackPrompt)) return null;
  const normalizedKind = String(kind || '').trim().toLowerCase();
  const phase = workflowSequencePhase(body);
  ...
  if (!/(Execution packet|実行パケット|Post draft|投稿案|Page structure|ページ構成|7-day execution sprint|7日実行スプリント|Media planner|Media plan|媒体設計|Funnel contract|計測・データ分析|Competitor teardown|競合ティアダウン|Acquisition research|顧客獲得リサーチ|Owner|Objective|Artifact|Metric|Stop rule|停止条件)/i.test(text)) return 'missing_cmo_execution_contract';
  return null;
}
